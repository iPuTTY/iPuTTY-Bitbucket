...
FontSpec *reg_read_setting_fontspec(void *handle, const char *name)
{
    char *settingname;
    char *fontname;
    FontSpec *ret;
    ...
    fontname = reg_read_setting_s(handle, name);
    if (!fontname)
	return NULL;

    settingname = dupcat(name, "IsBold", NULL);
    isbold = reg_read_setting_i(handle, settingname, -1);
    sfree(settingname);
    if (isbold == -1) {
        sfree(fontname);
        return NULL;
    ...
    settingname = dupcat(name, "CharSet", NULL);
    charset = reg_read_setting_i(handle, settingname, -1);
    sfree(settingname);
    if (charset == -1) {
        sfree(fontname);
        return NULL;
    ...
    settingname = dupcat(name, "Height", NULL);
    height = reg_read_setting_i(handle, settingname, INT_MIN);
    sfree(settingname);
    if (height == INT_MIN) {
        sfree(fontname);
        return NULL;
...
Filename *reg_read_setting_filename(void *handle, const char *name)
{
    char *tmp = reg_read_setting_s(handle, name);
    if (tmp) {
